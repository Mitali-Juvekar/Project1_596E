...
def process_image(inputs: list, parameters: dict) -> dict:
    ...
    if isinstance(inputs, list):
        image_paths = [input_item.file_path for input_item in inputs]  # Use dot notation
    else:
        return {"error": "Invalid input format"}, 400  # Return an error response if format is incorrect
    ...
    results_formatted = [ImageResult(file_path=path, result={'probability': float(prob), 'class': class_pred}) 
